...
| v3 | 2026-02-24 | Critical tone correction: "your expertise makes AI output good" (professional adoption). Honest scaffolding fields, three-part before/after, reflection captures context effort. |
| v4 | 2026-02-24 | Implementation notes refined per Israel's feedback: new route in Academy app (doesn't replace existing content); AI provider left to Claude Code to decide based on existing Prompts patterns; org context = Workbench context feature; department templates follow starter packs + i18n-ready pattern; added multi-language preparation requirement. |
| **v5** | **2026-02-24** | **Updated after design doc v6 decisions: prompt matching now references "all prompts in org's shared libraries" (no curated/shared distinction — employees already share prompts freely). Added feature-flag note for pilot rollout. Updated design doc reference to v6.** |
...
When generating use case cards, the AI also receives a summary of all prompts in the org's shared libraries (titles + descriptions + categories). There is no distinction between "curated templates" and "employee-shared prompts" — if a prompt is in a shared library, it's available for matching. For each use case, it attempts to match:
...
**Provider:** Follow the same AI integration pattern used in the Prompts app (e.g., for AI refinement, example runs). Claude Code should analyze the existing Prompts implementation to determine the right provider and patterns — do not assume Groq or any specific provider.
...
<prompt_library>
  {Titles + descriptions of all prompts in the org's shared libraries — no curated/shared distinction}
</prompt_library>
</context>
...
| **No prompt library matches** | All use case cards show "Create a prompt for this" with AI-suggested starter prompt. No auto-links to shared library prompts, but the creation path works. |
...
1. **Where does this live in the monorepo?** — This is a **new route in the Academy app**. It does NOT replace what currently exists in Academy (learning content, resources, links). The use case generator is an addition — a new section/route within the existing Academy app structure. Think of it as a new "Discover" area sitting alongside the existing learning content.

2. **AI integration patterns** — Do NOT assume a specific AI provider. Instead, analyze how the **Prompts app** currently handles AI calls (for AI refinement, example runs, etc.) and follow the same pattern, provider, and integration approach. The discriminated response types need reliable structured JSON output — look at how the Prompts app achieves this.

3. **Prompt library linking** — the prompt library is fully shipped. Employees can already share prompts to org libraries and create their own. There is **no distinction between "curated templates" and "employee-shared prompts"** — all prompts in the org's shared libraries are available for matching. The implementation needs to query all shared library prompts (titles + descriptions) to pass to the AI for matching. Understand the current prompt/library data model for the `linked_prompt_id` FK.

4. **Org context access** — The org-level data (industry, department, strategic priorities) that the wizard needs maps to what the **Workbench context feature** already provides. Analyze how Workbench context works to understand what org-level data is accessible from the employee's auth context, and reuse that pattern for injecting context into the wizard's AI prompts. Do not expose restricted data — only employee-safe context.
...
7. **Department templates seed migration** — 9 departments × 3-5 templates each = ~40 rows. Include `common_tasks` JSONB for seeding Step 1 suggestions. **IMPORTANT: Follow the same pattern used for "starter packs" and multi-language support.** The templates need to be structured so that they can be localized later without a painful migration. This means: translatable string fields should follow whatever i18n pattern the starter packs established, even though the wizard itself is English-only for v1. Prepare the data model and seed structure for multi-language from the start to avoid headaches when we add it later.

8. **The graceful degradation paths** — each fallback in the table above is a real code path that needs to exist. Test: what happens when the AI provider is down? When the org has no context? When the prompt library is empty?

9. **Feature flags** — The pilot rollout is full org at one client, controlled by feature flags. The Use Case Generator (and the broader Learn features) should be behind a feature flag that can be toggled per-organization or per-department. This is not complex — just ensure the routes and navigation entries respect a feature flag check.
