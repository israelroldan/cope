...
| **v11** | **2026-02-25** | **Expanded Interactive Learning Engine to 19 activity types. Added 10 mechanics from Tatoma learning format design: sorting buckets (classification), sequence builder (process ordering), binary stack (rapid accept/reject decisions), odd one out (pattern recognition), spot the error (critical analysis), associations (concept mapping), fill the gap (active recall), ranking (comparative judgment), multi match (vocabulary pairing), two-axis placement (quadrant analysis). Introduced shared ActivityEnvelope wrapper with universal scoring. Added mechanic selection guide mapping all 19 types to the 6 modules. Added Module 4 (Security) example lesson showcasing binary_stack and sorting_buckets. Mobile-first build order defined.** |
