...
| v9 | 2026-02-24 | Restructured 6 modules into 3 blocks: Foundation (AI basics → Use Case Generator → Try/Iterate/Save), Security (data privacy + responsible use), Advanced (AI tool landscape + agentic workflows). Separated content into its own worktree brief (worktree-academy-content.md) — engine and content are independent workstreams. By end of Foundation block, employee has used every core platform feature. |
| v10 | 2026-02-25 | Created Interactive Learning Engine worktree (worktree-interactive-learning.md) — Academy v2 evolution replacing markdown with interactive activities. 9 activity types (content card, annotated example, highlight select, drag-drop, label parts, multiple choice, open response with AI eval, guided prompt builder, prompt runner). Observe → Identify → Evaluate → Create progression. Use-case-connected learning. |
| v11 | 2026-02-25 | Expanded Interactive Learning Engine to 19 activity types. Added 10 mechanics from Tatoma learning format design: sorting buckets, sequence builder, binary stack, odd one out, spot the error, associations, fill the gap, ranking, multi match, two-axis placement. Introduced shared ActivityEnvelope wrapper with universal scoring. Added mechanic selection guide. Mobile-first build order defined. |
| v12 | 2026-02-25 | Restructured from 6 to 9 modules across 3 balanced paths (~1.5h each, ~4.5h total). Expanded Security from 1 module to 3: Data Privacy & What to Share, Spotting AI Mistakes, Responsible AI in Practice. Added capstone Module 9: Your AI Action Plan. Full interactive activity sequences for all 9 modules (88 activities total). Content worktree updated to v2 with learning objectives and content directions per module. |
| v13 | 2026-02-25 | Merged interactive learning engine + academy content into single worktree (worktree-academy-learning.md). Each module now includes learning objectives, content direction, and exit states alongside its activity sequence. Archived separate worktrees (worktree-interactive-learning-v3.md, worktree-academy-content-v2.md). Single handoff document for Sprint 3. |
| **v14** | **2026-02-25** | **Adopted Course → Lesson → Activity content hierarchy (aligned with LMS: SCORM Course→SCO→Interaction). "Learning Path"→"Course", "Module"→"Lesson". Added composition layer (Learning Paths reference Courses/Lessons for TL delivery). Matched worktree to implemented content: 3 courses (AI Foundations [markdown L1-3], Prompt Engineering [interactive L4-6], AI Strategy [interactive L7-9]). 81 interactive + 3 markdown = 9 lessons total.** |
...
### AI Foundations Path — The 6 Modules → [Full content worktree](./worktree-academy-content.md)

Each module follows the same structure:
1. **Concept** (3-5 min read) — one clear idea, conversational tone, real examples
2. **Exercise** (5-10 min) — hands-on task. Completing the exercise unlocks the next module. No graded quiz — the exercise IS the assessment.

The 6 modules are organized in three blocks: **Foundation** (1-3), **Security** (4), **Advanced** (5-6). The Foundation block is designed so that by module 3, the employee has used every core platform feature and created real personal data. Learning happens *through* the platform.

**Module 1: What AI Does (and Doesn't)** — FOUNDATION
- Concept: Mental model for LLMs. Good at: language tasks, summarizing, brainstorming. Bad at: facts, math, recent events.
- Exercise (prompt_execution): Try AI at something it's great at and something it's bad at. See the contrast.
- Takeaway: You know where AI shines and where it doesn't.

**Module 2: Find Your Use Cases** — FOUNDATION
- Concept: What makes a good AI use case. The "AI lens" for spotting opportunities in your daily work.
- Exercise (self_report): Run the Use Case Generator. Save at least 2 use cases.
- Takeaway: You've identified AI opportunities in your actual work.

**Module 3: Try a Prompt, Iterate, Save** — FOUNDATION
- Concept: Prompt anatomy (role, context, task, format). Why first output = draft. How to iterate. The value of saving refined prompts.
- Exercise (prompt_execution + self_report): Pick a use case from module 2, try a prompt, iterate with more context, save the prompt to your library, mark the use case as "Tried." Foundation complete.
- Takeaway: You have a personal prompt in your library that you built, tested, and refined with your own data.

**Module 4: Security & Responsible AI Use** — SECURITY
- Concept: Data privacy (where your data goes, what to never paste), hallucination risks, when NOT to use AI, org AI policy.
- Exercise (prompt_execution): Test AI's limitations — have it verify a factual claim and generate accuracy-critical content. See where it's confidently wrong.
- Takeaway: You know what to share and what not to, and why "trust but verify" matters.

**Module 5: The AI Tool Landscape** — ADVANCED
- Concept: What tools exist (ChatGPT, Perplexity, Midjourney, Copilot, etc.), what each is good at, org-approved tools if available. Matching use cases to tools.
- Exercise (self_report): Map your use cases to tools. Try one you haven't used before.
- Takeaway: You know which tool to reach for depending on the task.

**Module 6: Agentic Workflows** — ADVANCED
- Concept: Beyond single prompts — AI agents that take multiple steps, use tools, work through complex tasks. What agentic means, what's possible today, honest limitations, where this is heading.
- Exercise (self_report): Break down a multi-step use case into an ideal agentic workflow. Which steps could an agent handle? Which need your judgment?
- Takeaway: You understand where AI is going and can start thinking in terms of workflows, not just prompts.
